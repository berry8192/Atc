...
int main() {
    ...
    if (tob[0] < toa[0] && heada == 0) {
        cout << -1 << endl;
        return 0;
    }
    if (tob[tob.size() - 1] < toa[toa.size() - 1] && tailb == 0) {
        cout << -1 << endl;
        return 0;
    }
    ...
    // PV(tob);
    int ans = 0;
    int apos = 0, bpos = 0, aok = 0, bok = 0;
    while (aok == 0 || bok == 0) {
        // cout << toa[apos] SP << tob[bpos] << endl;
        ans++;
        apos++;
        bpos++;
        if (apos == toa.size()) {
