...
double use_time(int a, int b, int c, int d, double x) {
    // cout << "usetime: " << a << " " << b << " " << c << " " << d << " " << x
    //      << endl;
    return 1.0 * sqrt((a - c) * (a - c) + (b - d) * (b - d)) / x;
}
...
int main() {
    ...
    double la_t = 0;
    rep(i, n) { la_t += use_time(a[p[i]], b[p[i]], c[p[i]], d[p[i]], t); }
    double ans = lmax;
    do {
        // cout << lt SP << rt << endl;
        // PV(p);
        double lt = use_time(0, 0, a[p[0]], b[p[0]], s);
        double rt = use_time(0, 0, c[p[0]], d[p[0]], s);
        rep3(i, n, 1) {
            double l_l =
                use_time(a[p[i - 1]], b[p[i - 1]], a[p[i]], b[p[i]], s);
            double l_r =
                use_time(a[p[i - 1]], b[p[i - 1]], c[p[i]], d[p[i]], s);
            double r_l =
                use_time(c[p[i - 1]], d[p[i - 1]], a[p[i]], b[p[i]], s);
            double r_r =
                use_time(c[p[i - 1]], d[p[i - 1]], c[p[i]], d[p[i]], s);
            rt = min(lt + r_l, lt + r_r);
            lt = min(rt + l_l, rt + l_r);
            // swap(lt, rt);
            // cout << lt SP << rt << endl;
        }
