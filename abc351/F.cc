...
struct Compress {
    ...
    Compress(vector<int> vv, int indexd = 1) {
        ...
        for (auto itr = compress_set.begin(); itr != compress_set.end();
             itr++) {
            compress_map[*itr] = idx;
            idx++;
        }
        compress_v.resize(sz);
        rep(i, sz) { compress_v[i] = compress_map[vv[i]]; }
    }
};
map<int, int> mp;
...
int main() {
    ...
    rep(i, n) cin >> v[i];
    Compress com(v);
    rep(i, n) { mp[com.compress_v[i]] = v[i]; }
    // PV(com.compress_v);
    ...
    rep(i, n) {
        seg.set(com.compress_v[i], seg.get(com.compress_v[i]) + v[i]);
        seg2.set(com.compress_v[i], seg2.get(com.compress_v[i]) + 1);
        int prd = seg2.prod(0, com.compress_v[i]);
        ans += 1LL * v[i] * prd - seg.prod(0, com.compress_v[i]);
        // cout << ans << endl;
    }
