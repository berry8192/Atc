...
string solve(vector<int> t, vector<int> p, vector<string> s) {
    string server;
    vector<string> pc(n);
...
void gen_input(vector<int> &t, vector<int> &p, vector<string> &s) {
    rep(i, q) {
        t[i] = mt() % 3 + 1;
        p[i] = mt() % n + 1;
        if (t[i] == 2) {
...
int main() {
    ...
    string print_ans;
    repr(i, ans.size()) { print_ans += ans[i]; }
    string expected = solve(t, p, s);
    if (print_ans != expected) {
        cout << "WA" << endl;
        cout << n SP << q << endl;
        rep(i, q) {
            cout << t[i] SP << p[i] SP;
