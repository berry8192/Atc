...
int main() {
    ...
    rep(i, m) cin >> s[i] >> a[i];
    rep(i, l) cin >> t[i] >> b[i];
    vector<ll> aa = ruiseki(a);
    vector<ll> bb = ruiseki(b);

    set<int> event_times;
    map<int, pair<char, int>> taka, aoki;
    event_times.insert(0);
    int ti = 0;
    rep(i, m) {
        taka[ti] = {s[i], a[i]};
        ti += a[i];
        event_times.insert(ti);
    }
    ti = 0;
    rep(i, l) {
        aoki[ti] = {t[i], b[i]};
    ...
    vector<int> event_times_vec;
    for (auto et : event_times) {
        event_times_vec.push_back(et);
    }
    // for (auto et : event_times)
    //     cout << et SP;
    // cout << endl;
    rep(i, event_times_vec.size() - 1) {
        char current_dir = taka[event_times_vec[i]].first;
        int current_len = taka[event_times_vec[i]].second;

        if (taka.find(event_times_vec[i + 1]) == taka.end()) {
            taka[event_times_vec[i + 1]] = {
                current_dir, (event_times_vec[i + 1] - event_times_vec[i])};
            taka[event_times_vec[i]].second =
                current_len - (event_times_vec[i + 1] - event_times_vec[i]);
        }
    }
    rep(i, event_times_vec.size() - 1) {
        char current_dir = aoki[event_times_vec[i]].first;
        int current_len = aoki[event_times_vec[i]].second;
        if (aoki.find(event_times_vec[i + 1]) == aoki.end()) {
            aoki[event_times_vec[i + 1]] = {
                current_dir, (event_times_vec[i + 1] - event_times_vec[i])};
            aoki[event_times_vec[i]].second =
                current_len - (event_times_vec[i + 1] - event_times_vec[i]);
        }
    }
    for (auto takamp : taka) {
        cout << takamp.first << " " << takamp.second.first << " "
             << takamp.second.second << endl;
    }
    cout << endl;
    for (auto aokimp : aoki) {
        cout << aokimp.first << " " << aokimp.second.first << " "
             << aokimp.second.second << endl;
    }

    bool same_masu = (ra == rt && ca == ct);
    bool same_dir = true;
    bool crossed = false;
    ll ans = 0;
    int prev_time = 0;

    pair<char, int> prev_t = {s[0], a[0]};
    pair<char, int> prev_a = {t[0], b[0]};
    ...
    // for (auto m : mp) {
    //     int tim = m.first;
    //     pair<char, int> pt = m.second[0];
    //     pair<char, int> pa = m.second[1];
    //     if (pt.second == 0) {
    //         pt = prev_t;
    //     } else {
    //         prev_t = pt;
    ...
    //         prev_a = pa;
    //     }
    //     // cout << m.first << " " << pt.first << " " << pa.first << endl;
    //     if (same_masu) {
    //         if (same_dir) {
    //             ans += tim - prev_time;
    //         } else {
    ...
    //         }
    //     }
    //     same_dir = (pt.first == pa.first);
    //     int dt = dir_map[pt.first];
    //     int da = dir_map[pa.first];
    //     if (same_dir) {
    //         crossed = false;
    //     } else if ((dt + da) % 4 == 2) {
    //         if (pt.first == 'U') {
    //             if (ct == ca && rt > ra && (rt + ra) % 2 == 0) {
    //                 crossed = true;
    //             }
    //         } else if (pt.first == 'D') {
    //             if (ct == ca && rt < ra && (rt + ra) % 2 == 0) {
