...
struct Graph{
	...
	void init(int nn, int mm, bool weight=false, bool directed=false){
		...
		rep(i, mm){
			g[uu[i]].push_back({vv[i], 1});
			g[vv[i]].push_back({uu[i], 1});
		}
	}
	...
	bool bfs(int x){
		...
		while(!que.empty()){
			...
			for(int i=0;i<g[y].size();i++){
				int to_x=g[y][i].to;
				if(dist[to_x]!=imax && dist[to_x]%2==d%2) return false;
				if(d+g[y][i].co<dist[to_x]){
					dist[to_x]=d+g[y][i].co;
					que.push(to_x);
				}
			}
		}
