...
int main() {
    ...
    vector<int> r(q), c(q);
    set<pair<int, int>> st;
    map<pair<int, int>, pair<int, int>> le, ri, dw, up;

    rep(i, q) cin >> r[i] >> c[i];

    rep(i, q) {
        pair<int, int> pa = {r[i], c[i]};
        if (st.find(pa) == st.end()) {
            st.insert(pa);
            le[pa] = {r[i], max(0, c[i] - 1)};
            ri[pa] = {r[i], min(w - 1, c[i] + 1)};
            up[pa] = {max(0, r[i] - 1), c[i]};
            dw[pa] = {min(h - 1, r[i] + 1), c[i]};
        } else {
            pair<int, int> left = le[pa];
            while (1) {
                cout << "le" << endl;
                if (st.find(left) == st.end()) {
                    st.insert(left);
                    le[left] = {left.first, max(0, left.second - 1)};
                    ri[left] = {left.first, min(w - 1, left.second + 1)};
                    up[left] = {max(0, left.first - 1), left.second};
                    dw[left] = {min(h - 1, left.first + 1), left.second};
                    break;
                } else {
                    if (left.second != 0) {
                        left = le[left];
            ...
            pair<int, int> right = ri[pa];
            while (1) {
                cout << "ri" << endl;
                if (st.find(right) == st.end()) {
                    st.insert(right);
                    le[right] = {right.first, max(0, right.second - 1)};
                    ri[right] = {right.first, min(w - 1, right.second + 1)};
                    up[right] = {max(0, right.first - 1), right.second};
                    dw[right] = {min(h - 1, right.first + 1), right.second};
                    break;
                } else {
                    if (right.second != w - 1) {
                        right = ri[right];
            ...
            pair<int, int> down = dw[pa];
            while (1) {
                cout << "dw" << endl;
                if (st.find(down) == st.end()) {
                    st.insert(down);
                    le[down] = {down.first, max(0, down.second - 1)};
                    ri[down] = {down.first, min(w - 1, down.second + 1)};
                    up[down] = {max(0, down.first - 1), down.second};
                    dw[down] = {min(h - 1, down.first + 1), down.second};
                    break;
                } else {
                    if (down.first != 0) {
                        down = dw[down];
            ...
            pair<int, int> upp = up[pa];
            while (1) {
                cout << "up" << endl;
                if (st.find(upp) == st.end()) {
                    st.insert(upp);
                    le[upp] = {upp.first, max(0, upp.second - 1)};
                    ri[upp] = {upp.first, min(w - 1, upp.second + 1)};
                    up[upp] = {max(0, upp.first - 1), upp.second};
                    dw[upp] = {min(h - 1, upp.first + 1), upp.second};
                    break;
                } else {
                    if (upp.first != h - 1) {
                        upp = up[upp];
