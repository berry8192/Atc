...
const int MIN_DISTANCE_FOR_SPIRAL = 8;
...
vector<pair<int, int>> create_strategic_treants(int pi, int pj, int open_dir,
    ...
    // 花と冒険者の初期位置のマンハッタン距離をチェック
    int manhattan_dist =
        abs(ti - adventurer_start_r) + abs(tj - adventurer_start_c);
    bool use_distance_strategy = true; // 常に距離最大化戦略を使用
    ...
    // 距離最大化戦略：スタート地点からできるだけ遠い場所へ向かう
    // スタート地点に到達するか行き詰まるまで続ける
    while (true) {
        // スタート地点に到達した場合は終了
