...
int main() {
    ...
    vector<vector<ll>> dp(q + 1, vector<ll>(n, imax));
    if (h[0] == 'R') {
        dp[0][0] = 0;
    } else {
        dp[0][1] = 0;
    ...
    rep(i, q) {
        rep(j, n) {
            ...
            if (h[i] == 'L') {
                // left: l, right: j
                if ((j - l + n) % n > (t[i] - l + n) % n) {
                    dp[i + 1][j] =
                        min(dp[i + 1][j], dp[i][j] + (t[i] - l + n) % n);
                } else {
                    dp[i + 1][(t[i] + 1) % n] =
                        min(dp[i + 1][(t[i] + 1) % n],
                            dp[i][j] + (t[i] + 1 - prev_idx + n) % n +
                                (t[i] - l + n) % n);
                }
                if (n - (j - l + n) % n > n - (t[i] - l + n) % n) {
                    dp[i + 1][j] =
                        min(dp[i + 1][j], dp[i][j] + n - (t[i] - l + n) % n);
                } else {
                    dp[i + 1][(t[i] - 1 + n) % n] =
                        min(dp[i + 1][(t[i] - 1 + n) % n],
                            dp[i][j] + n - (t[i] - 1 - prev_idx + n) % n + n -
                                (t[i] - l + n) % n);
                }
                l = t[i];
            } else {
                // left: j, right: r
                if ((j - r + n) % n > (t[i] - r + n) % n) {
                    dp[i + 1][j] =
                        min(dp[i + 1][j], dp[i][j] + (t[i] - r + n) % n);
                } else {
                    dp[i + 1][(t[i] + 1) % n] =
                        min(dp[i + 1][(t[i] + 1) % n],
                            dp[i][j] + (t[i] + 1 - prev_idx + n) % n +
                ...
                if (n - (j - r + n) % n > n - (t[i] - r + n) % n) {
                    dp[i + 1][j] =
                        min(dp[i + 1][j], dp[i][j] + n - (t[i] - r + n) % n);
                } else {
                    dp[i + 1][(t[i] - 1 + n) % n] =
                        min(dp[i + 1][(t[i] - 1 + n) % n],
                            dp[i][j] + n - (t[i] - 1 - prev_idx + n) % n + n -
