...
vector<ll> rh(string s, int type) {
    vector<ll> rtn(s.size());
    ll hash = 0;
    ll base = 1;
    ll mod;
    if (type == 1) {
...
int main() {
    ...
    vector<int> ans(n);
    rep(i, n) {
        vector<ll> rh1 = rh(s[i], 1);
        vector<ll> rh2 = rh(s[i], 2);

        ans[i] = s[i].size();
        repr(j, s[i].size()) {
            // cout << rh1[j] SP << rh2[j] << endl;
            if (st1.find(rh1[j]) != st1.end() &&
                st2.find(rh2[j]) != st2.end()) {
                // cout << "hash: " << i SP << j << endl;
                int tmp = mp[rh1[j] * 1000000010 + rh2[j]];
                // cout << s[i].size() SP << tmp << endl;
                ans[i] = min(ans[i], int(s[i].size() - j + tmp - j - 2));
            }
        }
        ...
        rep(j, rh1.size()) {
            st1.insert(rh1[j]);
            st2.insert(rh2[j]);
            ll idx = rh1[j] * 1000000010 + rh2[j];
            if (mp.find(idx) != mp.end()) {
                mp[idx] = min(mp[idx], int(s[i].size()));
            } else {
                mp[idx] = s[i].size();
            }
        }
