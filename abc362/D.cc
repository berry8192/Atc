...
struct Graph {
    ...
    void djikstra(int startIndex) {
        ...
        dist[startIndex][startIndex] = a[startIndex];
        set<Edge> q;
        q.emplace(startIndex, -1, a[startIndex]);

        while (!q.empty()) {
            Edge e = *q.begin();
            const long long distance = e.cost;
            const int from = e.from;
            q.erase(q.begin());
            // cout << "from: " << from SP << "dist: " << distance << endl;
            ...
            // 現在の頂点からの各辺について
            rep(i, g[from].size()) {
                // cout << "from: " << g[from][i].from SP
                //      << "to: " << g[from][i].to SP
                //      << "cost: " << g[from][i].cost << endl;
                // to までの新しい距離
                const long long d = (dist[startIndex][from] + g[from][i].cost +
                                     a[g[from][i].to]);

                // d が現在の記録より小さければ更新
                // cout << "d: " << d
                //      << ", def: " << dist[startIndex][g[from][i].to] << endl;
                if (d < dist[startIndex][g[from][i].to]) {
                    dist[startIndex][g[from][i].to] = d;
                    q.emplace(g[from][i].to, -1, d);
                }
            }
        }
    }
...
int main() {
    ...
    ll ans = lmax;
    rep3(i, n, 1) { cout << g.dist[0][i] << endl; }
