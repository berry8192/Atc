...
void solve() {
    ...
    rep(i, q) { cin >> a[i] >> b[i]; }
    // cout << "q" << endl;
    vector<long double> rad(n), rad2(n), v(2 * n);
    rep(i, n) {
        int xx = abs(x[i]);
        int yy = abs(y[i]);
        if (xx == 0 && yy == 0) {
    ...
    // cout << v << endl;
    rep(i, q) {
        auto lb = lower_bound(all(v), rad[b[i] - 1]);
        long double rrad;
        if (rad[a[i] - 1] < rad[b[i] - 1]) {
            rrad = rad2[a[i] - 1];
        } else {
            rrad = rad[a[i] - 1];
        }
        auto rb = upper_bound(all(v), rrad);
        // cout << rad[b[i] - 1] << " " << rrad << endl;
        cout << rb - lb << endl;
    }
}
