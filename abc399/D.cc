...
int main() {
    ...
    rep(lp, t) {
        ...
        int ans = 0;
        rep(i, 2 * n - 1) {
            if (abs(rpl[v[i]] - rpr[v[i]]) <= 1) {
                continue;
            }
            if (abs(rpl[v[i + 1]] - rpr[v[i + 1]]) <= 1) {
                continue;
            }
            // cout << i SP << v[i] SP << v[i + 1] SP << rpl[v[i]] SP
            // << rpr[v[i]] SP << rpl[v[i + 1]] SP << rpr[v[i + 1]] << endl;
            if (abs(rpr[v[i]] - rpr[v[i + 1]]) <= 1 &&
                (i == rpl[v[i]] && i + 1 == rpl[v[i + 1]])) {
                ans++;
                // cout << "!" << endl;
            }
        }
