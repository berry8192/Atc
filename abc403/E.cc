...
struct Hash {
    ...
    bool operator<(const Hash &in) const {
        return h1 != in.h1 ? h1 < in.h1 : h2 != in.h2 ? h2 < in.h2 : h3 < in.h3;
    };
};

map<Hash, set<int>> rolling_hash(string s, int idx, bool is_x = false) {
    map<Hash, set<int>> rtn;
    ll mod1 = 1000000007;
    ll mod2 = 1000000009;
    ll mod3 = 998244353;
    ll base1 = 0;
    ...
    rep(i, s.size()) {
        ...
        d %= mod1;
        if (!is_x || i == s.size() - 1) {
            rtn[{base1, base2, base3}].insert(idx);
        }
    }
    return rtn;
}
...
map<Hash, set<int>> yh;
vector<string> s;
void del_hash(int idx) {
    map<Hash, set<int>> del_hashes = rolling_hash(s[idx], idx);
    for (auto mp : del_hashes) {
        {
            yh[mp.first].erase(idx);
        }
    }
}
...
int main() {
    ...
    rep(i, q) {
        ...
        if (t == 1) {
            map<Hash, set<int>> hashes = rolling_hash(s[i], i, true);
            for (auto hash : hashes) {
                xh.insert(hash.first);
                set<int> del_idxes = yh[hash.first];
                for (auto didx : del_idxes) {
                    del_hash(didx);
                    ans--;
                }
            }
            ...
            ans++;
            map<Hash, set<int>> hashes = rolling_hash(s[i], i);
            int ext = 0;
            for (auto hash : hashes) {
                for (auto idx : hash.second) {
                    yh[hash.first].insert(idx);
                    auto itr = xh.find(hash.first);
                    if (itr != xh.end()) {
                        del_hash(idx);
                        ans--;
                        ext = 1;
