...
int main() {
    ...
    vector<map<int, int>> v(d);
    rep(i, n) { v[a[i] % d][a[i] / d]++; }

    rep(i, d) {
        vector<vector<int>> dp(2, vector<int>(1000000 / d + 1, imax));
        dp[0][0] = v[i][0];
        dp[1][0] = 0;
        rep3(j, 1000000 / d + 1, 1) {
            dp[0][j] = min(dp[0][j - 1], dp[1][j - 1]) + v[i][j];
            dp[1][j] = dp[0][j - 1];
        }
        // cout << "i=" << i << endl;
        // rep(j, 10) { cout << dp[0][j] SP; }
        // cout << endl;
        // rep(j, 10) { cout << dp[1][j] SP; }
        // cout << endl;
        ans += min(dp[0][1000000 / d], dp[1][1000000 / d]);
    }
    cout << ans << endl;
