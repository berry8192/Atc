...
int main() {
    ...
    ll heikou = 0;
    for (auto m : a) {
        // cout << "a: " << m.first.first << " " << m.first.second << endl;
        map<ll, int> len;
        vector<pair<int, int>> p = m.second;
        ll sz = p.size();
        // cout << "size: " << sz << endl;
        ans += sz * (sz - 1) / 2;
        rep(i, p.size()) {
            int i1 = p[i].first;
            int i2 = p[i].second;
            ll tmp = (1LL * x[i1] - x[i2]) * (1LL * x[i1] - x[i2]) +
                     (1LL * y[i1] - y[i2]) * (1LL * y[i1] - y[i2]);
            len[tmp]++;
            // cout << "len: " << tmp << " i1: " << i1 << " i2: " << i2 << endl;
        }
        for (auto l : len) {
            int same = l.second;
            heikou += same * (same - 1) / 2;
            // cout << "length: " << l.first << " same: " << same << endl;
        }
    }

    cout << ans - heikou / 2 << endl;
