...
void solve() {
    ...
    dsu d(n);
    rep(i, v.size()) {
        if (!d.same(v[i].from, v[i].to)) {
            g.add_edge(v[i].from, v[i].to, v[i].cost);
            g.add_edge(v[i].to, v[i].from, v[i].cost);
            d.merge(v[i].from, v[i].to);
