...
int main() {
    ...
    rep(i, w) { sort(all(g[i])); }
    // PVV(g);

    int milen = imax;
    rep(i, w) { milen = min(milen, (int)g[i].size()); }
    vector<int> d(n, -1);
    rep(i, w) {
        rep(j, min(int(g[i].size()), milen)) { d[j] = max(d[j], g[i][j]); }
    }
    // PV(d);
    rep(i, q) {
        int tar_x = x[a[i]];
        auto idx = upper_bound(all(g[tar_x]), y[a[i]]);
        int index = idx - g[tar_x].begin() - 1;
        // cout << index << endl;
        if (d[index] == -1 || t[i] <= d[index]) {
            cout << "Yes" << endl;
        } else {
            cout << "No" << endl;
        }
