...
struct Figs {
    ...
    bool operator<(const Figs &in) const {
...
struct Fig {
    ...
    bool operator<(const Fig &in) const {
        return val != in.val ? val > in.val
               : x1 != in.x1 ? x1 < in.x1
               : x2 != in.x2 ? x2 < in.x2
                             : x3 < in.x3;
    };
...
int main() {
    ...
    set<Fig> st;
    Fig fig = {a[0] * b[0] + b[0] * c[0] + c[0] * a[0], 0, 0, 0};
    st.insert(fig);
    int cnt = 0;
    set<Figs> fs;
    ...
    while (!st.empty()) {
        ...
        int j = cur.x2;
        int k = cur.x3;
        // cout << cnt SP << cur.val SP << i SP << j SP << k << endl;
        cnt++;
        if (cnt == K) {
            cout << cur.val << endl;
            return 0;
        ...
        Figs fs2 = {i, j + 1, k};
        Figs fs3 = {i, j, k + 1};
        if (fs.find(fs1) == fs.end() && i + 1 < n) {
            Fig f1 = {a[i + 1] * b[j] + b[j] * c[k] + c[k] * a[i + 1], i + 1, j,
                      k};
            st.insert(f1);
            fs.insert(fs1);
        }
        if (fs.find(fs2) == fs.end() && j + 1 < n) {
            Fig f2 = {a[i] * b[j + 1] + b[j + 1] * c[k] + c[k] * a[i], i, j + 1,
                      k};
            st.insert(f2);
            fs.insert(fs2);
        }
        if (fs.find(fs3) == fs.end() && k + 1 < n) {
            Fig f3 = {a[i] * b[j] + b[j] * c[k + 1] + c[k + 1] * a[i], i, j,
                      k + 1};
            st.insert(f3);
            fs.insert(fs3);
        }
