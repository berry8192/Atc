...
int main() {
    ...
    rep(i, n) {
        rep(j, i + 1) {
            for (auto m : mp[j]) {
                // mp[i + 1][m.first] += m.second;
                mp[i + 1][m.first + v[i]] += m.second;
                if (m.first == -v[i]) {
                    minus[i + 1] += minus[max(0, j - 1)] * m.second;
                }
            }
        }
        rep(j, i + 1) {
