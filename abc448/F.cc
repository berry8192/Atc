...
void solve() {
    ...
    vector<vector<vector<int>>> blk(100, vector<vector<int>>(100));
    vector<int> x(n), y(n);
    rep(i, n) {
        cin >> x[i] >> y[i];
        blk[x[i] / 200001][y[i] / 200001].push_back(i + 1);
    }
    int cx = x[0], cy = y[0];
    vector<int> ans;
    rep(i, 100) {
        if (i % 2) {
            rrep(j, 100) {
                ...
                rep(k, blk[i][j].size()) {
                    v.push_back(
                        {x[blk[i][j][k]], y[blk[i][j][k]], -blk[i][j][k]});
                }
                sort(all(v));
    ...
    // ll tmp = 0;
    // int cx = x[ans[0] - 1], cy = y[ans[0] - 1];
    // rep(i, n - 1) {
    //     tmp += abs(cx - x[ans[i + 1]]) + abs(cy - y[ans[i + 1]]);
    //     cx = x[ans[i + 1]];
    //     cy = y[ans[i + 1]];
    // }
    // if (tmp > 10000000000LL) {
    // }
}
