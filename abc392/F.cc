...
int main() {
    ...
    rep(i, n) {
        ...
        rep(j, v.size()) {
            vidx = j;
            // cout << p[i] SP << v[j].size() << endl;
            if (p[i] <= v[j].size()) {
                break;
            }
            p[i] -= v[j].size();
        }
        v[vidx].insert(v[vidx].begin() + p[i], i + 1);
        // PV(v[vidx]);
        if (v[vidx].size() == 2000) {
            vector<int> tmp;
            rep3(k, v[vidx].size(), 1000) { tmp.push_back(v[vidx][k]); }
            rep(lp, 1000) { v[vidx].pop_back(); }
            v.insert(v.begin() + vidx + 1, tmp);
            // cout << "$" << endl;
            // cout << vidx << endl;
            // rep(ii, v.size()) {
            //     rep(jj, v[ii].size()) { cout << v[ii][jj] SP; }
            //     cout << endl;
            // }
            // cout << endl;
            // cout << "$$" << endl;
