...
struct Num {
    ...
    bool operator<(const Num &in) const {
        return pro != in.pro ? pro > in.pro : num < in.num;
    };
};
...
int main() {
    ...
    rep(i, n) {
        ...
        rep(j, k) {
            Num tmp = {1.0 * a[j] / su, j + 1};
            num.push_back(tmp);
        }
    }
    sort(all(num));
    rep(i, num.size()) { cout << num[i].num SP << num[i].pro << endl; }
    vector<double> prob(100010);
    rep(i, 100010) {
        auto nu = num[i];
        if (prob[nu.num] != 0) {
            cout << prob[nu.num] * nu.pro << endl;
            return 0;
        }
        prob[nu.num] = nu.pro;
    }
