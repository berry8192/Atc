...
struct Sl {
    ...
    bool operator<(const Sl &in) const {
        return st != in.st ? st < in.st : h != in.h ? h < in.h : w < in.w;
    };
};
...
int main() {
    ...
    rep(i, 4) {
        ...
        st.insert({v[nh][nw], nh, nw});
    }
    // for (auto s : st) {
    //     cout << s.st << " " << s.h << " " << s.w << endl;
    // }
    // return 0;
    while (!st.empty()) {
        auto itr = st.begin();
        Sl sl = *itr;
        st.erase(itr);
        // cout << sl.st SP << sl.h SP << sl.w << endl;
        double t = ans * 1.0;
        double s = 1.0 * sl.st * x;
        if (sl.st * x >= ans || s >= t) {
            break;
        }
        ans += sl.st;
