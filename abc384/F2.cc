...
struct Pa {
    ...
    bool operator<(const Pa &in) const {
        return na != in.na ? na > in.na : sc < in.sc;
    };
};
...
int main() {
    ...
    rep(k, 26) {
        ...
        rep(i, n) {
            ll rv = (inf - v[i]) % two_k;
            pair<ll, ll> p = mp[rv];
            mp[rv] = {p.first + 1, p.second + v[i]};
            int idx = v[i] % two_k;
            p = mp[idx];
            dk[k] += p.first * v[i] + p.second;
        }
    }
    vector<ll> d(30);
    rep(i, 29) { d[i] = dk[i] - dk[i + 1]; }
    // PV(dk);
    rep(i, 29) { ans += d[i] / (1 << i); }
