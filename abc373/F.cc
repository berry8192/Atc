...
int main() {
    int N, W;
    ll score = 0, best_score = 0;
    int weight = 0;
    vector<int> w, v, cho, dup;
    vector<double> d;
    set<pair<ll, int>> st;
    ...
    while (timer.progress() < 1) {
        ...
        auto itr = st.begin();
        int nx = mt() % min(N, int(st.size())) - 1;
        rep(i, nx) {
            itr++;
            // if (weight + w[(*itr).second] > W) {
            //     itr--;
            //     break;
            // }
        }
        ...
        if (weight + wei <= W) {
            ...
            weight += wei;
            cho[idx]++;
            int nget = (v[idx] - (dup[cho[idx] + 1] - dup[cho[idx]]));
            if (0 < nget) {
                st.insert({-nget, idx});
            }
            if (best_score < score) {
            ...
            // cout << "val: " << val << endl;
            // cout << "wei: " << wei << endl;
            // cout << "cho[idx]: " << cho[idx] << endl;
            // cout << "new: " << v[idx] - (dup[cho[idx] + 1] - dup[cho[idx]])
            //      << endl;
        } else {
            // cout << idx << endl;
            // cout << "val: " << val << endl;
            ...
            if (mt() % 5 == 0 || st.size() == 1) {
                ...
                // while (cho[del] == 0)
                //     del = (del + 1) % N;
                // score -= cho[del] * v[del] - dup[cho[del]];
                // weight -= cho[del] * w[del];
                // st.erase(
                //     {-(v[del] - (dup[cho[del] + 1] - dup[cho[del]])), del});
                // cho[del] = 0;
                // st.insert({-(v[del] - 1), del});
            }
        }
    }
