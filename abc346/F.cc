...
int main() {
    ...
    cin >> s;
    cin >> t;
    vector<vector<ll>> v(26, vector<ll>(s.size()));
    vector<vector<ll>> w(26);
    ...
    ll le = 0, ri = 1000000000000000010, mi;
    while (ri - le > 1) {
        mi = (le + ri) / 2;
        // mi = 2;
        cout << "mi:::::::: " << mi SP << le SP << ri << endl;
        ...
        rep(i, t.size()) {
            int c = t[i] - 'a';
            cout << "t: " << c << endl;
            cout << "nn, pos: " << nn SP << pos << endl;
            ll rem = mi;
            // if (w[c][t.size()] - w[c][pos] >= rem) {
            ll tmp = w[c][pos] + rem;
            auto lb = lower_bound(all(w[c]), tmp);
            if (lb == w[c].end()) {
                rem -= w[c][s.size()] - w[c][pos];
                pos = 0;
                nn++;
            } else {
                pos = lb - w[c].begin();
            ...
            // cout << rem SP << w[c][t.size()] << endl;
            ll loop = rem / w[c][s.size()];
            nn += loop;
            rem -= loop * w[c][s.size()];
            // cout << "Rem : " << rem << endl;

            // remは0~t.size-1
            lb = lower_bound(all(w[c]), rem);
            pos = lb - w[c].begin();
            // cout << "nn: " << nn << endl;
            if (nn >= n) {
                ok = 0;
