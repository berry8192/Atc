...
struct Masu {
    ...
    bool operator>(const Masu &in) const { return dep > in.dep; };
};
...
int main() {
    int h, w, k;
    cin >> h >> w >> k;
    vector<vector<int>> board(h, vector<int>(w, imax));
    vector<vector<int>> queued(h, vector<int>(w, 0));
    ...
    priority_queue<Masu, vector<Masu>, greater<Masu>> que;
    rep(i, h) {
        rep(j, w) {
            if (board[i][j] != imax) {
                continue;
    ...
    while (!que.empty()) {
        ...
        rep(d, 4) {
            ...
            if (queued[nh][nw]) {
                continue; // すでにキューに入っている
            }
            vector<int> v;
            rep(dd, 4) {
                int nnh = nh + d4h[dd];
                int nnw = nw + d4w[dd];
                if (nnh < 0 || nnh >= h || nnw < 0 || nnw >= w)
                    continue;
                if (board[nnh][nnw] != imax) {
                    v.push_back(board[nnh][nnw]);
                }
            }
            sort(all(v));
            if (v.size() >= 2) {
                que.push({nh, nw, v[1] + 1});
                queued[nh][nw] = 1;
            }
        }
    }
