...
bool touhi(vector<int> v) {
    rep(i, v.size() - 2) {
        if (1LL * v[i + 1] * v[i + 1] != 1LL * v[i] * v[i + 2]) {
            return false;
        }
    }
    return true;
...
int main() {
    ...
    rep(lp, t) {
        ...
        if (vm.size() == vp.size() || vm.size() == vp.size() - 1) {
            vector<int> v;
            rep(i, vm.size() + vp.size()) {
                if (i % 2 == 0) {
                    v.push_back(vp[i / 2]);
                } else {
                    v.push_back(vm[i / 2]);
        ...
        if (vp.size() == vm.size() || vp.size() == vm.size() - 1) {
            vector<int> v;
            rep(i, vp.size() + vm.size()) {
                if (i % 2 == 0) {
                    v.push_back(vm[i / 2]);
                } else {
                    v.push_back(vp[i / 2]);
