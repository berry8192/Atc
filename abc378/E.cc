...
int main() {
    ...
    vector<ll> rrv(n + 1), orrv(n + 1);
    rep(i, n) {
        rrv[i + 1] = rrv[i] + rv[i + 1];
        orrv[i + 1] = orrv[i] + orv[i + 1];
    }
    PV(rrv);
    PV(orrv);

    rep(i, n) {
        // cout << rrv[n] SP << rrv[i] SP << rv[i] SP << n - i << endl;
        ll tmp = rrv[n] - rrv[i] - (rv[i] * (n - i)) + (orrv[i]) * m;
        ans += tmp;
        cout << tmp << endl;
    }
