...
struct kc {
    ...
    bool operator<(const kc &in) const { return c < in.c; };
};
...
int main() {
    ...
    dsu d(n);
    // rep(i, m) { cout << v[i].c SP << v[i].k << endl; }
    // rep(i, n) cout << d.leader(i) << endl;
    // rep(i, m) {
    //     rep(j, v[i].k) {
    //         cout << "v[" << i << "].a[" << j << "]=" << v[i].a[j] << endl;
    //     }
    // }
    // return 0;
    ...
    rep(i, m) {
        ...
        rep(j, v[i].k - 1) {
            // cout << "v[i].a[j]=" << v[i].a[j] << endl;
            if (!d.same(v[i].a[j], v[i].a[j + 1])) {
                // if (seen[v[i].a[j]] && seen[v[i].a[j + 1]])
                //     continue;
                ans += v[i].c;
                // cout << "#" SP << v[i].a[j] SP << d.leader(v[i].a[j]) <<
                // endl; cout << v[i].a[j] SP << ans << endl;
                d.merge(v[i].a[j], v[i].a[j + 1]);
            }
