...
void dfs(bitset<64> bs, int pl, int pi) {
    // cout << bs << endl;
    if (bs.count() == 2 * n) {
        obj.push_back(edges);
        return;
...
int main() {
    ...
    rep(i, obj.size()) {
        bitset<64> tbs;
        rep(j, obj[i].size()) {
            tbs.set(obj[i][j].first * n + obj[i][j].second);
            tbs.set(obj[i][j].second * n + obj[i][j].first);
        }
        mi = min(mi, int((bs ^ tbs).count()));
    }
    cout << mi / 2 << endl;
