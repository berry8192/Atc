...
int main() {
    ...
    ll l, r, not_p = 0, p_pow = 0;
    cin >> l >> r;

    rep(i, pri.size()) {
        not_p += r / pri[i] - (l - 1) / pri[i];
        p_pow += floor(log(r) / log(pri[i])) - floor(log(l - 1) / log(pri[i]));
    }

    cout << "not_p: " << not_p << endl;
    cout << "p_pow: " << p_pow << endl;
    cout << r - l + 1 - not_p + p_pow << endl;
