...
void solve() {
    ...
    rep(i, 500010) {
        if (i == 0) {
            ...
            vector<int> l1 = {-i * 2 + 1, -i * 2, -i * 2, i * 2},
                        r1 = {i * 2, -i * 2, i * 2 - 1, i * 2},
                        d1 = {i * 2, -i * 2 + 1, -i * 2, -i * 2},
                        u1 = {i * 2, i * 2, -i * 2, i * 2 - 1};
            rep(j, 4) {
                ll tl = max(l1[j], l);
                ll tr = min(r1[j], r);
                ll td = max(d1[j], d);
                ll tu = min(u1[j], u);
                // cout << tl << " " << tr << " " << td << " " << tu << endl;
                ll tmp = max(0LL, tu - td + 1) * max(0LL, tr - tl + 1);
                // cout << i << " " << tmp << endl;
                ans += tmp;
            }
        }
