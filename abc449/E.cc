...
void solve() {
    ...
    rep(i, q) { xs.push_back({x[i], i}); };
    sort(all(xs));

    vector<int> d(m + 1);
    rep(i, n) { d[a[i]]++; }
    vector<pair<int, int>> v;
    rep(i, m) { v.push_back({d[i + 1], i + 1}); }
    sort(all(v));
    ...
    rep(i, m) {
        if (i < m - 1) {
            ...
            step.push_back(1LL * diff * w.size());
        } else {
            w.push_back(v[i].second);
        }
    }
    // cout << step << endl;
    vector<ll> ruiseki = cumulative_sum(step);
    // cout << ruiseki << endl;
    // cout << w << endl;
    ...
    rep(i, q) {
        ll xx = xs[i].first;
        auto lb = lower_bound(all(ruiseki), xx);
        ll idx, md;
        if (lb == ruiseki.end()) {
            idx = ruiseki.size() - 1;
            if (idx != ws) {
                cerr << "sorted" << endl;
                for (int j = ws; j < idx; j++) {
                    ww.push_back(w[j]);
                }
                ws = ww.size();
                sort(all(ww));
            ...
            // cout << idx  << " " << md << endl;
            // cout << ww[md) % idx] << endl;
            ans[xs[i].second] = ww[md % idx];
        } else {
            if (*lb == n) {
                ans[xs[i].second] = a[xx - 1];
            } else {
                idx = lb - ruiseki.begin() - 1;
                if (idx != ws) {
                    cerr << "sorted" << endl;
                    for (int j = ws; j < idx; j++) {
                        ww.push_back(w[j]);
