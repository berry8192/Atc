// AHC066 お片付けロボット — D.cc
// C.cc + 以下:
//   - 方向aware Dijkstra による min(F + 回転) 経路で eval/build
//     (BFSは「最短ステップ」だが回転コストは付かないため、振り回しが多い)
//   - インラインマクロ登録: 本体の長い F-run を M..M で囲むだけにして
//     プロローグ(M F^ml M + (0,0)復帰)を省く
//   - SA高速化: dijk_table[src*4+dir][dst] = (cost, end_dir) 直接参照
...
int main() {
    ...
    int ml_lock = best_ml;
    const double T_SA2 = 1.85;
    runSA(perm2, curLen2, bestPerm2, bestLen2,
          [&ml_lock](const vector<int>& p){ return evalPermLenApprox(p, ml_lock); },
          T_SA2, rng);

    cerr << "N=" << N << " M=" << M << " T=" << T << " best_ml=" << best_ml << " bestLen2=" << bestLen2 << endl;

    // 候補集合
    vector<vector<int>> perm_set = {bestPerm, bestPerm2, naive_perm, nn_perm};
