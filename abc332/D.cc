...
int main(){
	...
	vector<map<int, int>> mpha(h), mphb(h), mpwa(w), mpwb(w);
	rep(i, h){
		rep(j, w){
			mpha[i][a[i][j]]++;
			mphb[i][b[i][j]]++;
	...
	vector<vector<int>> jh(h, vector<int>(h)), jw(w, vector<int>(w));
	rep(i, h){
		rep(j, h){
			if(mpha[i]==mphb[j]) jh[i][j]=1;
			else jh[i][j]=0;
		}
	}
	rep(i, w){
		rep(j, w){
			if(mpwa[i]==mpwb[j]) jw[i][j]=1;
			else jw[i][j]=0;
		}
	}
	...
	while(ansh && !qh.empty()){
		...
		rep(i, h-1){
			...
			vector<int> tmp=ch;
			swap(tmp[i], tmp[i+1]);
			if(sth.find(tmp)==sth.end()){
				sth.insert(tmp);
				int flg=1;
				rep(j, h){
					if(jh[j][tmp[j]]==0) flg=0;
				}
				if(flg){
					ansh=dh+1;
					fnd=1;
	...
	while(answ && !qw.empty()){
		...
		rep(i, w-1){
			...
			vector<int> tmp=cw;
			swap(tmp[i], tmp[i+1]);
			if(stw.find(tmp)==stw.end()){
				stw.insert(tmp);
				int flg=1;
				rep(j, w){
					if(jw[j][tmp[j]]==0) flg=0;
				}
				if(flg){
					answ=dw+1;
					fnd=1;
