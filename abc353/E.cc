...
int main() {
    ...
    rep(i, 300010) {
        ...
        for (auto idx : st) {
            // cout << s[idx][i] << endl;
            // cout << prv SP << s[idx][i] << " sep: " << sep[idx] << endl;
            if (prv == 'A') {
                prv = s[idx][i];
            } else {
                if (s[idx][i] != prv || sep[idx]) {
                    // if (sep[idx] == 0)
                    ans += 1LL * streak * (streak + 1) / 2;
                    streak = 0;
                    prv = s[idx][i];
                    sep[idx] = 1;
                } else {
        ...
        rep(j, del.size()) st.erase(del[j]);
        if (st.empty())
            break;

        // rep(j, 10) { cout << sep[j] SP; }
        // cout << endl;
    }
