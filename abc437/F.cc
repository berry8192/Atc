...
ll e() { return -2100000007; }
ll e2() { return 2100000007; }
...
void solve() {
    ...
    segtree<ll, op, e> seglM(il), segrM(ir);
    segtree<ll, op2, e2> seglm(il), segrm(ir);
    // cout << seglM.prod(0, 2) << endl;
    // cout << segrM.prod(0, 2) << endl;
    // cout << seglm.prod(0, 2) << endl;
    // cout << segrm.prod(0, 2) << endl;
    ...
    rep(lp, q) {
        ...
        if (t == 1) {
            ...
            ll cr = xx - yy;
            ll ans = -1;
            ans = max(ans, abs(seglM.prod(L, R) - cl));
            ans = max(ans, abs(segrM.prod(L, R) - cr));
            ans = max(ans, abs(seglm.prod(L, R) - cl));
            ans = max(ans, abs(segrm.prod(L, R) - cr));
            cout << ans << endl;
        }
