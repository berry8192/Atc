...
int main() {
    ...
    set<int> used, used_pos;
    rep(i, n) {
        if (pos[i].empty())
            continue;
        if (pos[i][pos[i].size() - 1] != i) {
            cout << 0 << endl;
            return 0;
        }
        if (pos[i].size() >= 2) {
            rep3(j, pos[i].size(), 1) used.insert(pos[i][j]);
            rep(j, pos[i].size() - 1) used_pos.insert(pos[i][j]);
        }
    }
    // PS(used);
    // PS(used_pos);

    vector<int> non_used, non_used_pos;
    rep(i, n) {
        if (used.find(i) == used.end()) {
            non_used.push_back(i);
        }
        if (used_pos.find(i) == used_pos.end()) {
            non_used_pos.push_back(i);
        }
    }
    // PV(non_used);
    // PV(non_used_pos);
    ...
    mint ans = 1;
    int cnt = 0;
    rep(i, non_used_pos.size()) {
        // cout << "non " << i SP << v[i] << endl;
        auto lb = lower_bound(all(non_used), non_used_pos[i] + 1);
        // cout << "non_used.size()-non_used.end() - lb <<
        // non_used_pos.size() - (non_used_pos.end() - lb) << endl;
        int tmp = non_used.size() - (non_used.end() - lb) - cnt;
        ans *= tmp;
        // cout << tmp << endl;
        cnt++;
    }
