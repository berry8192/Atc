...
struct Graph {
    ...
    int dfs(int v, int dep, int prev) {
        cout << v SP << dep SP << prev << endl;
        if (seen[v]) {
            return -1;
        }
        if (dep == k) {
            ...
            if (g[v].size() == 2) {
                int nxt;
                if (g[v][0].to == prev) {
                    g[v].erase(g[v].begin() + 0);
                    nxt = g[v][1].to;
                } else {
                    g[v].erase(g[v].begin() + 1);
                    nxt = g[v][0].to;
                }
                if (g[nxt].size() == 2) {
                    if (g[nxt][0].to == v) {
                        g[nxt].erase(g[nxt].begin() + 0);
                    } else {
                        g[nxt].erase(g[nxt].begin() + 1);
                    }
                }
            }
            return -2;
