...
int main() {
    ...
    rep(lp, t) {
        ...
        rep(bi, 60) {
            ...
            // dp[i桁目まで決まっている][Nより小さいか][1の個数]
            mint dp[61][2][61];
            dp[0][0][0] = 1;
            rep(i, 60) {
                rep(j, K + 1) {
                    // cout << bi SP << i SP << j << endl;
                    ...
                    int ni = (N & (1LL << (60 - 1 - i))) ? 1 : 0;
                    // i桁目までNと同じで、i+1桁目はNより小さい数の時
                    rep(k, ni) { dp[i + 1][1][j + k] += dp[i][0][j]; }
                    //  i桁目までNと同じで、i+1桁目もNと同じ数の時
                    if (i == bi && ni == 1) {
                        continue;
                    }
                    dp[i + 1][0][j + ni] = dp[i][0][j];
                }

                // rep(j, 3) {
                //     cout << dp[i][0][j].val() SP << dp[i][1][j].val() <<
                //     endl;
                // }
            }
            // cout << ans.val() << endl;
            ans += (dp[60][0][K] + dp[60][1][K]) * bit;
            cout << (dp[60][0][K] + dp[60][1][K]).val() << endl;
        }
        cout << ans.val() << endl;
    }
