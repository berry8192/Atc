...
struct Dist {
    ...
    bool operator>(const Dist &in) const {
        return dist != in.dist ? dist > in.dist
               : v1 != in.v1   ? v1 < in.v1
                               : v2 < in.v2;
    };
};
...
int main() {
    ...
    rep(i, n) rep(j, i) pq.push({abs(x[i] - x[j]) + abs(y[i] - y[j]), i, j});
    rep(i, n) {
        rep(j, i) {
    ...
    rep(lp, q) {
        ...
        if (t == 1) {
            ...
            y[vsize] = b;
            rep(i, vsize) {
                int dist = abs(x[i] - a) + abs(y[i] - b);
                pq.push({dist, i, vsize});
            }
            vsize++;
        } else if (t == 2) {
            int flg = 1;
            ...
            while (!pq.empty()) {
                Dist dist = pq.top();
                if (changed != -1 && changed != dist.dist) {
                    break;
                }
                pq.pop();

                if (!d.same(dist.v1, dist.v2)) {
                    d.merge(dist.v1, dist.v2);
                    changed = dist.dist;
                }
            }
            cout << changed << endl;
