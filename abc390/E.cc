...
int main() {
    int n, x;
    vector<int> v, a, c, a1, a2, a3, c1, c2, c3;
    cin >> n >> x;
    v.resize(n);
    a.resize(n);
    c.resize(n);
    ...
    ll le = -1, ri = 1000000100, mi;

    while (ri - le > 1) {
        mi = (ri + le) / 2;
        // cout << le SP << ri SP << mi << endl;
        int rest_cal = x;
        int flg;
        vector<vector<int>> dp1(a1.size() + 1, vector<int>(rest_cal + 1));
        rep(i, a1.size()) {
            repr(j, rest_cal + 1) {
                dp1[i + 1][j] = dp1[i][j];
                if (j + c1[i] <= rest_cal) {
                    dp1[i + 1][j + c1[i]] =
                        max(dp1[i + 1][j + c1[i]], dp1[i][j] + a1[i]);
                }
            }
        }
        flg = 1;
        ...
        vector<vector<int>> dp2(a2.size() + 1, vector<int>(rest_cal + 1));
        rep(i, a2.size()) {
            repr(j, rest_cal + 1) {
                dp2[i + 1][j] = dp2[i][j];
                if (j + c2[i] <= rest_cal) {
                    dp2[i + 1][j + c2[i]] =
                        max(dp2[i + 1][j + c2[i]], dp2[i][j] + a2[i]);
                }
            }
        }
        flg = 1;
        ...
        vector<vector<int>> dp3(a3.size() + 1, vector<int>(rest_cal + 1));
        rep(i, a3.size()) {
            repr(j, rest_cal + 1) {
                dp3[i + 1][j] = dp3[i][j];
                if (j + c3[i] <= rest_cal) {
                    dp3[i + 1][j + c3[i]] =
                        max(dp3[i + 1][j + c3[i]], dp3[i][j] + a3[i]);
                }
            }
        }
        flg = 1;
