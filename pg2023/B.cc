...
int main(){
	...
	double ans=0;
	int div=0;
	// vector<vector<int>> dp(s.size()+1, vector<int>(4));
	// dp[0][0]=1;
	...
	// 		// dp[i+1][2]/=3;
	// 	}
	// 	cout<< dp[i+1][0] SP << dp[i+1][1] SP << dp[i+1][2] SP << dp[i+1][3] <<endl;
	// }
	// ans=dp[s.size()][3];
	// rep(i, div) ans/=3.0;
	...
	rep(i, s.size()){
		...
		// cout<< dp[i+1][0] SP << dp[i+1][1] SP << dp[i+1][2] SP << ans <<endl;
		cout<< dp[i+1] <<endl;
	}
	rep(i, div) ans/=3.0;
	cout<< ans <<endl;

	// vector<vector<double>> dp(s.size()+1, vector<double>(4));
	// dp[0][0]=1.0;
