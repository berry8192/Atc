...
void dfs(vector<int> v, bool sma, bool lar) {
    // if (sma)
    //     cout << "small";
    // if (lar)
    //     cout << "large";
    ...
    for (int i = li; i <= ri; i++) {
        bool is_sma = (sma || i != ri);
        bool is_lar = (lar || i != li);
        if (is_sma && is_lar) {
            PV(v);
            int rest = 18 - idx;
            int usable;
            ...
            ll tmp = 1;
            rep(j, rest) { tmp *= usable; }
            ans += tmp;
            cout << i SP << rest SP << usable SP << tmp << endl;
            continue;
        }
        v.push_back(i);
        if (v.size() == 19) {
