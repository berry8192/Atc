...
struct Graph {
    ...
    void board_init(int h, int w, int &sh, int &sw, int &gh, int &gw, int par) {
        hh = h;
        ww = w;
        // #.形式のboardを想定
        // 頂点番号は2重forの順に0から
            ...
            for (int j = 0; j < w; j++) {
                if ((i + j) % 2 == par) {
                    ...
                    if (s[i][j] != '#' && i + 1 < h && s[i + 1][j] != '#') {
                        g[i * w + j].push_back({i * w + j, (i + 1) * w + j, 1});
                        // cout << i SP << j SP << "D" << endl;
                    }
                    if (s[i][j] != '#' && i - 1 >= 0 && s[i - 1][j] != '#') {
                        g[i * w + j].push_back({i * w + j, (i - 1) * w + j, 1});
                        // cout << i SP << j SP << "U" << endl;
                    }
                } else {
                    // 横のみ
                    if (s[i][j] != '#' && j + 1 < w && s[i][j + 1] != '#') {
                        g[i * w + j].push_back({i * w + j, i * w + j + 1, 1});
                        // cout << i SP << j SP << "R" << endl;
                    }
                    if (s[i][j] != '#' && j - 1 >= 0 && s[i][j - 1] != '#') {
                        g[i * w + j].push_back({i * w + j, i * w + j - 1, 1});
                        // cout << i SP << j SP << "L" << endl;
                    }
                }
            }
        }
...
int main() {
    ...
    Graph g0;
    g0.board_init(h, w, sh, sw, gh, gw, 0);
    vector<int> dist0 = g0.bfs(sh * w + sw);
    ans = min(ans, dist0[gh * w + gw]);

    Graph g1;
    g1.board_init(h, w, sh, sw, gh, gw, 1);
    vector<int> dist1 = g1.bfs(sh * w + sw);
    ans = min(ans, dist1[gh * w + gw]);
