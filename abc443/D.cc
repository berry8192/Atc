...
void solve() {
    ...
    rep(i, n) {
        int lp = 1;
        while (v[i].second - lp >= 0) {
            int idx = v[i].second - lp;
            if (lseen[idx]) {
                break;
            }
            if (le[idx] >= v[i].first + lp) {
                le[idx] = v[i].first + lp;
                lseen[idx] = 1;
            } else {
                break;
    ...
    rep(i, n) {
        int lp = 1;
        while (v[i].second + lp < n) {
            int idx = v[i].second + lp;
            if (rseen[idx]) {
                break;
            }
            if (ri[idx] >= v[i].first + lp) {
                ri[idx] = v[i].first + lp;
                rseen[idx] = 1;
            } else {
                break;
    ...
    // cout << ri << endl;
    ll ans = 0;
    rep(i, n) { ans += max(0, a[i] - min(le[i], ri[i])); }
    cout << ans << endl;
}
