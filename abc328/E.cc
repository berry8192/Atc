...
struct Graph{
	...
	void dfs(vector<int> vv, ll w, ll hash){
		if(vv.size()==n){
			// PV(vv);
			// cout<< w <<endl;
			ans=min(ans, w%k);
		...
		rep(j, vv.size()){
			...
			for(int i=0;i<g[x].size();i++){
				if(find(vv.begin(), vv.end(), g[x][i].to)==vv.end()){
					vector<int> vvv=vv;
					vvv.push_back(g[x][i].to);
					// if(st.find(hash+(w+g[x][i].co)*vvv.size()%k)!=st.end()) continue;
					dfs(vvv, w+g[x][i].co, (hash+(w+g[x][i].co)*vvv.size()%k));
				}
			}
		}
	}
	void dfsp(int prev, vector<int> per, ll w){
		if(per.empty()){
			ans=min(ans, w%k);
			return;
		}
		int x=per[per.size()-1];
		per.pop_back();
		rep(i, g[x].size()){
			if(g[x][i].to=prev){
				prev=x;
				dfsp(prev, per, w+g[x][i].co);
			}
		}
	}
	void perm(){
