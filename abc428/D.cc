...
int main() {
    ...
    rep(lp, t) {
        ...
        ll ans = 0;
        ll l, r;
        ll lmin = 1, rmax = 9, ten = 10;
        while (lmin <= c + d) {
            l = max(lmin, c + 1);
            r = min(rmax, c + d);
            // cout << "lr: " << l SP << r << endl;
            // cout << sqrt_count(c * ten + r) SP << sqrt_count(c * ten + l)
            //      << endl;
            if (l <= r) {
                ans += sqrt_count(c * ten + r) - sqrt_count(c * ten + l - 1);
            }
            lmin *= 10;
            rmax = (rmax + 1) * 10 - 1;
            ten *= 10;
