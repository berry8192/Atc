...
struct Graph {
    ...
    void djikstra(int startIndex) {
        dist.clear();
        dist.resize(n, 100000000);
        dist[startIndex] = 0;
        priority_queue<Edge, vector<Edge>, greater<Edge>> q;
        q.emplace(startIndex, startIndex, 0);
        ...
        while (!q.empty()) {
            ...
            for (const auto &edge : g[from]) {
                // to までの新しい距離
                const long long d = (dist[from] + edge.cost);

                // d が現在の記録より小さければ更新
                if (d < dist[edge.to]) {
                    dist[edge.to] = d;
                    q.emplace(edge.to, edge.to, d);
                }
            }
        }
    }
    void djikstra2(int startIndex) {
        dist2.clear();
        dist2.resize(n, 100000000);
        dist2[startIndex] = 0;
        priority_queue<Edge, vector<Edge>, greater<Edge>> q;
        q.emplace(startIndex, startIndex, 0);

        while (!q.empty()) {
            const long long dist2ance = q.top().cost;
            const int from = q.top().from;
            q.pop();

            // 最短距離でなければ処理しない
            if (dist2[from] < dist2ance) {
                continue;
            }
            ...
            for (const auto &edge : g[from]) {
                // to までの新しい距離
                const long long d = (dist2[from] + edge.cost);
...
int main() {
    ...
    rep(i, n) {
        if (graph.dist[i] == graph.dist2[i]) {
            cout << max(m1, m2) << endl;
        } else if (graph.dist[i] < graph.dist2[i]) {
            cout << m2 << endl;
        } else {
            cout << m1 << endl;
        }
