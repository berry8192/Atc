...
int main() {
    ...
    vector<vector<ll>> rui(primes.size());
    rep(i, primes.size()) {
        ll num = primes[i] * primes[i];
        ll current = num;
        while (current <= 1000000000000) {
            rui[i].push_back(current);
            if (log10(1.0 * current * num) > 12) {
                break;
            }
            current *= num;
        }
    ...
    // int cnt = 0;
    // rep(i, rui.size()) { cnt += rui[i].size(); }
    // cout << cnt << endl;
    // return 0;
    set<ll> ruis;
    for (int i = rui.size() - 1; i >= 0; i--) {
        repr(j, rui[i].size()) {
            auto itr = ruis.begin();
            while (itr != ruis.end()) {
                // cout << *itr << " " << rui[i][j] << endl;
                if (log10(1.0 * rui[i][j] * (*itr)) > 12) {
                    break;
                }
                // cout << rui[i][j] SP << ruis[k] << endl;
                num400.push_back(rui[i][j] * (*itr));
                itr++;
            }
        }
        repr(j, rui[i].size()) { ruis.insert(rui[i][j]); }
        // cerr << i << " " << ruis.size() << endl;
    }
    sort(all(num400));
    // PV(num400);

    rep(i, q) {
        auto itr = upper_bound(all(num400), v[i]);
        itr--;
        cout << *itr << endl;
    }
