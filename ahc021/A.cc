...
struct Pyra{
	...
	int calc_score(){
		if(mov.size()==0) return 11111;
		else return mov.size();
	}
	void print_ans(){
		cout<< mov.size() <<endl;
		rep(i, mov.size()) cout<< mov[i].first.h SP << mov[i].first.w SP << mov[i].second.h SP << mov[i].second.w <<endl;
	}
};
...
int main(){
    ...
    while (true){
		...
		// 	rep(i, n-1){
		// 		rep(j, i+1){
		// 			if(pyra.random_move(i, j)){
		// 				flag=0;
		// 				// cout<< i SP << j <<endl;
		// 			}
		// 			if(pyra.mov.size()>10000){
		// 				cout<< "over" <<endl;
		// 				break;
		// 			}
		// 		}
