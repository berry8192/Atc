...
int main() {
    ...
    sort(all(v));
    // rep(i, n) { cout << v[i].first SP << v[i].second << endl; }
    // return 0;
    rep(i, n) {
        int idx = v[i].second;
        // cout << "idx: " << idx << endl;
        auto ritr = wall.lower_bound(idx);
        auto litr = wall.lower_bound(idx);
        litr--;
        int r = *ritr;
        int l = *litr;
        int dist = r - l - 1;
        ...
        int acc_idx = n - dist;
        // cout << dist SP << ldis SP << rdis << endl;
        vel[acc_idx] += -v[i].first;
        vel[acc_idx + ldis] += v[i].first;
        vel[acc_idx + rdis] += v[i].first;
    }
    // PV(vel);
    vector<ll> ans = ruiseki(vel);
    vector<ll> ans2 = ruiseki(ans);
    vector<ll> outv;

    // rep(i, n) { cout << ans[i + 1] SP; }
    // cout << endl;
    // rep(i, n) { cout << ans2[i + 2] SP; }
    // cout << endl;
    rep(i, n) { outv.push_back(ans2[i + 2]); }
    reverse(all(outv));
    rep(i, n) cout << outv[i] << endl;
