...
int main() {
    ...
    rep(i, n) {
        ...
        rep(j, sg[i].size()) {
            d.merge(u[sg[i][j]], v[sg[i][j]]);
            // if (u[sg[i][j]] < v[sg[i][j]]) {
            //     merged++;
            // }
        }
        // cout << all_e.size() SP << i SP << d.size(i) << endl;
        if (d.size(i) == i + 1) {
            ans[i] = all_e.size() - d.size(i);
        } else {
            ans[i] = -1;
        }
    }
