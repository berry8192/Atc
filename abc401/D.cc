...
void rle(string s, vector<tuple<char, int, int>> &vec) {
    int cnt = 1;
    for (int i = 1; i < (int)s.size(); i++) {
        if (s[i] != s[i - 1]) {
            if (s[i - 1] == '?') {
                vec.push_back({s[i - 1], cnt, i - cnt});
            }
            cnt = 0;
        }
        cnt++;
    ...
    if (s.back() == '?') {
        vec.push_back({s.back(), cnt, s.size() - cnt});
    }
}
...
int main() {
    ...
    // cout << t << endl;
    vector<tuple<char, int, int>> hatena;
    rle(t, hatena);
    rep(i, hatena.size()) {
        // cout << hatena[i].first << hatena[i].second << endl;
    }
