...
void dfs(string s, vector<mint> &dp) {
    int tmp = 0;
    rep(i, s.size()) {
        if (s[i] == '?') {
            s[i] = 'A';
...
void PVmint(vector<mint> vm, int sz) {
    sz = min(sz, int(vm.size()));
    rep(i, sz) { cout << vm[i].val() << " "; }
    cout << endl;
}

string bin2(int x, int sz) {
    string rtn = "";
    rep(i, sz) {
        rtn += (x % 2 ? "A" : "B");
        x /= 2;
...
int main() {
    ...
    rep3(i, n, k) {
        vector<mint> hal(1030);
        rep(j, (1 << k)) { hal[j / 2] += dp[i][j]; }
        // PVmint(hal, (1 << (k - 1)));
        if (s[i] == '?') {
            rep(j, (1 << (k - 1))) {
                if (!kaibun(bin2(j + (1 << (k - 1)), k))) {
                    dp[i + 1][j + (1 << (k - 1))] += hal[j];
                }
                if (!kaibun(bin2(j, k))) {
                    dp[i + 1][j] += hal[j];
                }
