...
int main() {
    int n, m, l;
    cin >> n >> m >> l;
    vector<int> a(n), init_add(n);
    rep(i, n) cin >> a[i];

    rep(i, 510) rep(j, 510) dp[i][j] = 0;

    rep(i, n) {
        rep3(j, i + 1, max(0, i - l + 1)) { init_add[j] += a[i]; }
    }
    rep(i, n) { init_add[i] = m - (init_add[i] % m); }
    PV(init_add);
    rep(i, init_add[0] + 1) {
        dp[0][i] = i;
        cout << "dp[" << 0 << "][" << i << "]=" << dp[0][i] SP;
    }
    cout << endl;
    ...
    rep(i, n) {
        rep(j, init_add[i] + 1) {
            int tmp = imax;
            rep(l, j + 1) { tmp = min(tmp, dp[i][l] + j - l); }
            dp[i + 1][j] = tmp;
            cout << "dp[" << i + 1 << "][" << j << "]=" << dp[i + 1][j] SP;
        }
        cout << endl;
    }
