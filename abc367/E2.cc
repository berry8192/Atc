...
ll k;
vector<int> x, a;
vector<int> bel;            // どのmapに属するか
vector<map<int, int>> vmpv; // どの数字がiターン目
vector<vector<int>> vmpk;   // iターン目にどの数字
vector<int> lop;            // ループの始点idx
vector<int> wiv, wii;       // どこに合流するか、何番目のidxか

void dfs(int bas, int val, int dep) {
    // cout << bas SP << val SP << dep << endl;
    if (bel[val] != bas && bel[val] != -1) {
        return;
    }
    if (vmpv[bas][val] != 0) {
        lop[bas] = vmpv[bas][val] - mar;
        return;
    }
    bel[val] = bas;
    vmpv[bas][val] = dep + mar;
    vmpk[bas].push_back(val);
    dfs(bas, x[val], dep + 1);
}
...
int main() {
    ...
    rep(i, n) {
        rep(j, vmpk[i].size()) {
            ans[vmpk[i][j]] = vmpk[i][(j + k) % vmpk[i].size()];
            dfsr()
        }
    }
