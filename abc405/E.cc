...
int main() {
    ...
    int d;
    cin >> d;
    vector<mint> ab(b + 1), cd(c + 1);
    rep(i, b + 1) { ab[i] = fac[a - 1 + b - i] / fac[a - 1] / fac[b - i]; }
    rep(i, c + 1) {
        // cout << "i: " << i << endl;
        // cout << fac[d - 1 + c].val() << endl;
        // cout << fac[d - 1].val() << endl;
        // cout << fac[c - i].val() << endl;
        cd[i] = fac[d - 1 + c - i] / fac[d - 1] / fac[c - i];
    }
    // rep(i, b + 1) { cout << ab[i].val() SP; }
    // cout << endl;
    // rep(i, c + 1) { cout << cd[i].val() SP; }
    // cout << endl;
    mint ans = 0;
    rep(i, b + 1) {
        rep(j, c + 1) { ans += ab[i] * cd[j] * (fac[i + j] / fac[i] / fac[j]); }
    }
    cout << ans.val() << endl;
    // rep(i, b + 1) {
